...
class RecordTextFieldState extends State<RecordTextField> {
  ...
  Record? _applySlashCommand(String text) {
    ...
    // Start from a clean slate: same id/date/position, empty content
  ...
  void _handleTextChange() {
    ...
    if (!widget.readOnly && text.startsWith('/') && text.trim().length > 1) {
      ...
      if (converted != null) {
        ...
        // Dispatch synchronously so RecordSection can addPostFrameCallback
        // before the rebuild happens; the new FocusNode registers itself during
        // the build, and the callback fires after — restoring keyboard focus.
